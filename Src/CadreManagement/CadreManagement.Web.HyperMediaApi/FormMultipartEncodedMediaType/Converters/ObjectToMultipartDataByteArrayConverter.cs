...
using System.Text;
using CadreManagement.Web.HyperMediaApi.FormMultipartEncodedMediaType.Extensions;
using static System.String;

namespace CadreManagement.Web.HyperMediaApi.FormMultipartEncodedMediaType.Converters
{
    public class ObjectToMultipartDataByteArrayConverter
    {
        public byte[] Convert(object value, string boundary)
        {
            ...
            if (IsNullOrWhiteSpace(boundary))
        ...
        private void FillFlatPropertiesListFromObject(object obj, string prefix, List<KeyValuePair<string, object>> propertiesList)
        {
            if (obj != null)
            {
                ...
                if (obj is IDictionary)
                {
                    ...
                    {
                        string indexedKeyPropName = Format("{0}[{1}].Key", prefix, index);
                        FillFlatPropertiesListFromObject(key, indexedKeyPropName, propertiesList);

                        string indexedValuePropName = Format("{0}[{1}].Value", prefix, index);
                ...
                else if (obj is ICollection)
                {
                    ...
                    {
                        string indexedPropName = Format("{0}[{1}]", prefix, index);
                ...
                else if (type.IsCustomNonEnumerableType())
                {
                    ...
                    {
                        string propName = IsNullOrWhiteSpace(prefix)
                                              ? propertyInfo.Name
                ...
                foreach (var param in postParameters)
                {
                    ...
                    else
                    {
                        ...
                        if (param.Value != null)
                        {
                            ...
                            {
                                throw new Exception(Format("Type \"{0}\" cannot be converted to string", param.Value.GetType().FullName));
                            }
                        }
